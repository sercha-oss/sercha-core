...
//
// The previous test suite asserted typed fields on CapabilityPreferences
// (TextIndexingEnabled, EmbeddingIndexingEnabled, etc.) that no longer
// exist — preferences are now a flat Toggles map keyed by CapabilityType.
// The full suite needs to be rebuilt against the new shape (toggle map
// in/out, registry-driven service injection). Deferred to a tester pass.
